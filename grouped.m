...
for rate=1:5 %epsilon2:epsilon3 = 1:5, 1:4, 1:3, 1:2, 1:1
    ...
    %     filename3=strcat(str3, num2str(ep));
        ...
        map=[5,14,9,7,8,2,10,1,12,4,3,13,11,6];%第1个属性是第5个进入N的(el)
        %map=[1,14,5,2,13,6,8,3,12,4,10,9,11,7];%(正常)
        %map=[8,10,14,1,6,13,5,7,4,9,11,12,3,2];
        %map=[];%属性编号i对应进入网络顺序map(i)
        ...
        for index = 1:d %属性编号+1
            if index==2||index==4||index==5||index==7||index==8||index==10||index==13
        ...
        save('dataset_new11.mat',dataset_new)
        ...
        aver_var_dist_av=sum(temp)/50;
        avd3{rate}(ep)=aver_var_dist_av;
        ...
        accuracy_av=sum(accuracy)/50;
        SVM{rate}(ep,class)=accuracy_av; %rate: epsilon2:epsilon3; ep:0.2,0.4,...; class:class_attribute:education/gender
        end

    end
end

save("avd3_11.mat",'avd3')
save("avd2_11.mat",'avd2')
save("SVM11.mat","SVM")
...
% end
% 
% save("avd2_2.mat",'avd2')
% save("avd3_2.mat",'avd3')
% save("SVM_4.mat","SVM")